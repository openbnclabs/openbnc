...
| Tình huống | Lệnh |
|----------|---------|
| Có API key, muốn cài nhanh nhất | `openbnc onboard --api-key sk-... --provider openrouter` |
| Muốn được hướng dẫn từng bước | `openbnc onboard --interactive` |
| Đã có config, chỉ cần sửa kênh | `openbnc onboard --channels-only` |
...
- Thiết lập nhanh: `openbnc onboard --api-key "sk-..." --provider openrouter`
- Thiết lập tương tác: `openbnc onboard --interactive`
- Kiểm tra môi trường: `openbnc status` + `openbnc doctor`
