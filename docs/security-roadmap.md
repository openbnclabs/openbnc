# openbnc Security Improvement Roadmap
...
openbnc already has **excellent application-layer security**:
...
## Comparison: openbnc vs PicoClaw vs Production Grade

| Feature | PicoClaw | openbnc Now | openbnc + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`openbnc audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/openbnc/audit.log"
sign_events = true
max_size_mb = 100
...
openbnc security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
openbnc audit --user @alice --since 24h
openbnc audit --risk high --violations-only
openbnc audit --verify-signatures

# Sandbox test
openbnc sandbox --test
# → Testing isolation...
...
**openbnc is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, openbnc becomes:
...
**Value**: Transforms openbnc from "safe for testing" to "safe for production"
