...
This page defines the fastest supported path to install and initialize openbnc.
...
Default behavior is **app-only** (build/install openbnc) and expects existing Rust toolchain.
...
```bash
curl -fsSL https://raw.githubusercontent.com/openbnclabs/openbnc/main/scripts/install.sh | bash
```
...
This builds a local openbnc image and launches onboarding inside a container while
persisting config/workspace to `./.openbnc-docker`.

Container CLI defaults to `docker`. If Docker CLI is unavailable and `podman` exists,
bootstrap auto-falls back to `podman`. You can also set `OPENBNC_CONTAINER_CLI`
explicitly (for example: `OPENBNC_CONTAINER_CLI=podman ./bootstrap.sh --docker`).
...
Docker tag (`OPENBNC_DOCKER_IMAGE`, default: `openbnc-bootstrap:local`); if missing,
it pulls `ghcr.io/openbnclabs/openbnc:latest` and tags it locally before running.
...
```bash
OPENBNC_API_KEY="sk-..." OPENBNC_PROVIDER="openrouter" ./bootstrap.sh --onboard
```
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/openbnclabs/openbnc:latest`)
- `--skip-install`
- `--provider <id>`
