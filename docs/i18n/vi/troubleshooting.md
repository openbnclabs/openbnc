...
- `cargo check` / `cargo build` dừng lâu ở `Checking openbnc`
...
### Không tìm thấy lệnh `openbnc` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `openbnc`
...
- Dừng các tiến trình `openbnc daemon` / `openbnc channel start` thừa
