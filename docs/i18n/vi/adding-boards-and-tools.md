# Thêm Board và Tool — Hướng dẫn phần cứng openbnc

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào openbnc.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.openbnc/config.toml)
openbnc peripheral add nucleo-f401re /dev/ttyACM0
openbnc peripheral add arduino-uno /dev/cu.usbmodem12345
openbnc peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, openbnc có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `openbnc peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `openbnc peripheral list` | Liệt kê các board đã cấu hình |
| `openbnc peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `openbnc peripheral flash` | Nạp firmware Arduino |
| `openbnc peripheral flash-nucleo` | Nạp firmware Nucleo |
| `openbnc hardware discover` | Liệt kê thiết bị USB |
| `openbnc hardware info` | Thông tin chip qua probe-rs |
