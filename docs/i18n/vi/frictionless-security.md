...
[INFO] ✓ Audit logging enabled (~/.config/openbnc/audit.log)
...
$ openbnc security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/openbnc/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ openbnc config set security.sandbox.enabled false

# Bật backend cụ thể
$ openbnc config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ openbnc config set security.resources.max_memory_mb 2048
```
...
```bash
$ openbnc onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Hiển thị trạng thái** — `openbnc security --status`
