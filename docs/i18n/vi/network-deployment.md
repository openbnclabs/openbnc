# Triển khai mạng — openbnc trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai openbnc trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | openbnc poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | openbnc sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — openbnc thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. openbnc trên Raspberry Pi
...
- Channel Telegram hoạt động: openbnc poll Telegram API (outbound)
...
```bash
openbnc daemon --host 0.0.0.0 --port 3000
```
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `openbnc tunnel` (xem tài liệu tunnel).

3. openbnc sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- openbnc gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `openbnc daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `openbnc daemon`).
...
- [ ] Chạy `openbnc daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
