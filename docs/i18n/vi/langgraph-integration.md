...
Hướng dẫn này giải thích cách sử dụng gói Python `openbnc-tools` để gọi tool nhất quán với bất kỳ LLM provider nào tương thích OpenAI.

## Bối cảnh

Một số LLM provider, đặc biệt là các model Trung Quốc như GLM-5 (Zhipu AI), có hành vi gọi tool không nhất quán khi dùng phương thức text-based tool invocation. Core Rust của openbnc sử dụng structured tool calling theo định dạng OpenAI API, nhưng một số model phản hồi tốt hơn với cách tiếp cận khác.
...
```python
import asyncio
from openbnc_tools import create_agent, shell, file_read, file_write
...
```python
from openbnc_tools import tool, create_agent
...
```python
import os
from openbnc_tools.integrations import DiscordBot
...
# Single message
openbnc-tools "What is the current date?"

# Interactive mode
openbnc-tools -i
```

## So sánh với Rust openbnc

| Khía cạnh | Rust openbnc | openbnc-tools |
|--------|---------------|-----------------|
