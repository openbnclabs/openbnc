# Network Deployment — openbnc on Raspberry Pi and Local Network

This document covers deploying openbnc on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | openbnc polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | openbnc syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — openbnc connects to external servers/relays. No port forwarding or public IP required.

---

## 2. openbnc on Raspberry Pi
...
```bash
openbnc daemon --host 127.0.0.1 --port 42617
```

- Gateway binds to `127.0.0.1` — not reachable from other machines
- Telegram channel works: openbnc polls Telegram API (outbound)
...
```bash
openbnc daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `openbnc tunnel` (see tunnel docs).

3. openbnc will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- openbnc calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `openbnc daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `openbnc daemon` service).
...
- [ ] Run `openbnc daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
openbnc supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `openbnc` system user (created during install)

### 7.2 Install Service

```bash
# Install service (OpenRC is auto-detected on Alpine)
sudo openbnc service install
```

This creates:
- Init script: `/etc/init.d/openbnc`
- Config directory: `/etc/openbnc/`
- Log directory: `/var/log/openbnc/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo openbnc service install` automatically prepares `/etc/openbnc`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `openbnc` service user.

If no prior runtime state is available to migrate, create `/etc/openbnc/config.toml` before starting the service.

### 7.4 Enable and Start

```bash
# Add to default runlevel
sudo rc-update add openbnc default

# Start the service
sudo rc-service openbnc start

# Check status
sudo rc-service openbnc status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service openbnc start` | Start the daemon |
| `sudo rc-service openbnc stop` | Stop the daemon |
| `sudo rc-service openbnc status` | Check service status |
| `sudo rc-service openbnc restart` | Restart the daemon |
| `sudo openbnc service status` | openbnc status wrapper (uses `/etc/openbnc` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/openbnc/access.log` |
| Errors/stderr | `/var/log/openbnc/error.log` |

View logs:

```bash
sudo tail -f /var/log/openbnc/error.log
```
...
sudo rc-service openbnc stop
sudo rc-update del openbnc default

# Remove init script
sudo openbnc service uninstall
```
...
- The service runs as the `openbnc:openbnc` user (least privilege)
- Config must be at `/etc/openbnc/config.toml` (explicit path in init script)
- If the `openbnc` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo openbnc service install`
- [ ] Enable: `sudo rc-update add openbnc default`
- [ ] Start: `sudo rc-service openbnc start`
- [ ] Verify: `sudo rc-service openbnc status`
- [ ] Check logs: `/var/log/openbnc/error.log`
