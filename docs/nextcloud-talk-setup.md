...
This guide covers native Nextcloud Talk integration for openbnc.
...
Add this section in `~/.openbnc/config.toml`:
...
openbnc daemon
# or
openbnc gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, openbnc verifies:
...
- openbnc ignores bot-originated webhook events (`actorType = bots`).
- openbnc ignores non-message/system events.
...
3. Confirm openbnc receives and replies in the same room.
