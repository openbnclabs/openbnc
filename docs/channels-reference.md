...
This document is the canonical reference for channel configuration in openbnc.
...
6. **Stale runtime state**: config changed but `openbnc daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.openbnc/config.toml`.
...
When running `openbnc channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `openbnc models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

openbnc supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `openbnc channel list`, `openbnc channel doctor`, and `openbnc channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@openbnc:matrix.example.com"   # optional, recommended for E2EE
...
openbnc supports two WhatsApp backends:
...
```toml
[channels_config.whatsapp]
session_path = "~/.openbnc/state/whatsapp-web/session.db"
...
port = 6697
nickname = "openbnc-bot"
username = "openbnc"              # optional
channels = ["#openbnc"]
allowed_users = ["*"]
...
- `OPENBNC_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `OPENBNC_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `openbnc daemon` after config changes.
...
```bash
RUST_LOG=info openbnc daemon 2>&1 | tee /tmp/openbnc.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/openbnc.log
```
