# Adding Boards and Tools — openbnc Hardware Guide

This guide explains how to add new hardware boards and custom tools to openbnc.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.openbnc/config.toml)
openbnc peripheral add nucleo-f401re /dev/ttyACM0
openbnc peripheral add arduino-uno /dev/cu.usbmodem12345
openbnc peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, openbnc can index PDF files:
...
2. **Add to config** — `openbnc peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `openbnc peripheral list` | List configured boards |
| `openbnc peripheral add <board> <path>` | Add board (writes config) |
| `openbnc peripheral flash` | Flash Arduino firmware |
| `openbnc peripheral flash-nucleo` | Flash Nucleo firmware |
| `openbnc hardware discover` | List USB devices |
| `openbnc hardware info` | Chip info via probe-rs |
