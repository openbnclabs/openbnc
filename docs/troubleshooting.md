...
- `cargo check` / `cargo build` appears stuck at `Checking openbnc` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in openbnc:
...
### `openbnc` command not found after install

Symptom:

- install succeeds but shell cannot find `openbnc`
...
- stop extra `openbnc daemon` / `openbnc channel start` processes
