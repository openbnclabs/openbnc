...
|---|---|---|
| Foreground runtime | `openbnc daemon` | gỡ lỗi cục bộ, phiên ngắn |
| Foreground gateway only | `openbnc gateway` | kiểm thử webhook endpoint |
| User service | `openbnc service install && openbnc service start` | runtime được quản lý liên tục bởi operator |
...
|---|---|---|
| Tính hợp lệ của config | `openbnc doctor` | không có lỗi nghiêm trọng |
| Kết nối channel | `openbnc channel doctor` | các channel đã cấu hình đều khoẻ mạnh |
| Tóm tắt runtime | `openbnc status` | provider/model/channels như mong đợi |
| Heartbeat/trạng thái daemon | `~/.openbnc/daemon_state.json` | file được cập nhật định kỳ |
...
4. Nếu các channel vẫn thất bại, kiểm tra allowlist và thông tin xác thực trong `~/.openbnc/config.toml`.
...
1. sao lưu `~/.openbnc/config.toml`
2. chỉ áp dụng một thay đổi logic tại một thời điểm
3. chạy `openbnc doctor`
