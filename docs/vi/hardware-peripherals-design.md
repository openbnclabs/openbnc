# Thiết kế Hardware Peripherals — openbnc

openbnc cho phép các vi điều khiển (MCU) và máy tính nhúng (SBC) **phân tích lệnh ngôn ngữ tự nhiên theo thời gian thực**, tổng hợp code phù hợp với từng phần cứng, và thực thi tương tác với ngoại vi trực tiếp.

## 1. Tầm nhìn

**Mục tiêu:** openbnc đóng vai trò là AI agent có hiểu biết về phần cứng, cụ thể:
...
**Hình dung trực quan:** openbnc = bộ não hiểu phần cứng. Ngoại vi = tay chân mà nó điều khiển.
...
openbnc chạy **trực tiếp trên thiết bị**. Board khởi động server gRPC/nanoRPC và giao tiếp với ngoại vi ngay tại chỗ.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  openbnc on ESP32 / Raspberry Pi (Edge-Native)                             │
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────────┐ │
...
2. openbnc truy xuất tài liệu theo board (ví dụ: bản đồ GPIO của ESP32)
...
openbnc chạy trên **máy chủ** và duy trì kết nối phần cứng tới thiết bị mục tiêu. Dùng cho phát triển, kiểm tra nội tâm, và nạp firmware.

```
┌─────────────────────┐                    ┌──────────────────────────────────┐
│  openbnc on Mac    │   USB / J-Link /   │  STM32 Nucleo-F401RE              │
...
2. openbnc nhận diện phần cứng đang kết nối (VID/PID, kiến trúc)
...
2. openbnc ghi/nạp firmware qua OpenOCD hoặc probe-rs
3. Xác nhận thành công

**Hoặc:**
1. openbnc tự phát hiện: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|-----------|-------------|---------------|
| openbnc chạy trên | Thiết bị (ESP32, RPi) | Máy chủ (Mac, Linux) |
...
Máy chủ chạy openbnc; ngoại vi chạy firmware tối giản. JSON đơn giản qua serial.

### Chế độ B: RPi làm Host (Native GPIO)

openbnc trên Pi; GPIO qua rppal hoặc sysfs. Không cần firmware riêng.
...
board = "esp32"
transport = "wifi"
# Edge-Native: openbnc runs on ESP32
```
...
1. **Khởi động:** openbnc nạp config, đọc `peripherals.boards`.
...
Dành cho RPC có kiểu dữ liệu, độ trễ thấp giữa openbnc và các peripheral:
...
- **openbnc-firmware** hoặc **openbnc-peripheral** — một crate/workspace riêng biệt.
...
- Người dùng nạp lên board; openbnc kết nối và tự phát hiện khả năng.

## 9. Các giai đoạn triển khai

### Phase 1: Skeleton ✅ (Hoàn thành)

- [x] Thêm trait `Peripheral`, config schema, CLI (`openbnc peripheral list/add`)
- [x] Thêm flag `--peripheral` cho agent
- [x] Ghi tài liệu vào AGENTS.md

### Phase 2: Host-Mediated — Phát hiện phần cứng ✅ (Hoàn thành)

- [x] `openbnc hardware discover`: liệt kê thiết bị USB (VID/PID)
- [x] Board registry: ánh xạ VID/PID → kiến trúc, tên (ví dụ: Nucleo-F401RE)
- [x] `openbnc hardware introspect <path>`: memory map, danh sách peripheral
...
- [x] openbnc trên Raspberry Pi (native GPIO qua rppal)
...
- [x] Crate firmware `openbnc-esp32` (`firmware/openbnc-esp32`) — GPIO qua UART
- [x] ESP32 trong hardware registry (CH340 VID/PID)
- [ ] openbnc *chạy trực tiếp trên* ESP32 (WiFi + LLM, edge-native) — tương lai
- [ ] Thực thi Wasm hoặc dựa trên template cho logic do LLM tạo ra

**Cách dùng:** Nạp `firmware/openbnc-esp32` vào ESP32, thêm `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` vào config.
...
- Chạy openbnc đầy đủ *trực tiếp trên* STM32 bare-metal (không có WiFi, RAM hạn chế) — dùng Host-Mediated thay thế
...
> *"Các board như ESP, Raspberry Pi, hoặc các board có WiFi có thể kết nối với LLM (Gemini hoặc mã nguồn mở). openbnc chạy trên thiết bị, tạo gRPC riêng, khởi động nó, và giao tiếp với ngoại vi. Người dùng hỏi qua WhatsApp: 'di chuyển cánh tay X' hoặc 'bật LED'. openbnc lấy tài liệu chính xác, viết code, thực thi, lưu trữ tối ưu, chạy, và bật LED — tất cả trên board phát triển.*
>
> *Với STM Nucleo kết nối qua USB/J-Link/Aardvark vào Mac: openbnc từ Mac truy cập phần cứng, cài đặt hoặc ghi những gì cần thiết lên thiết bị, và trả kết quả. Ví dụ: 'Hey openbnc, những địa chỉ khả dụng/đọc được trên thiết bị USB này là gì?' Nó có thể tự tìm ra thiết bị nào đang kết nối ở đâu và đưa ra gợi ý."*
