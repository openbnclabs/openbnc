...
openbnc supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export OPENBNC_API_KEY="your-api-key"
openbnc agent
```

## llama.cpp Server (Recommended Local Setup)

openbnc includes a first-class local provider for `llama-server`:
...
```bash
openbnc models refresh --provider llamacpp
openbnc agent -m "hello"
```

You do not need to export `OPENBNC_API_KEY=dummy` for this flow.

## SGLang Server

openbnc includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
openbnc models refresh --provider sglang
openbnc agent -m "hello"
```
...
openbnc includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
openbnc models refresh --provider vllm
openbnc agent -m "hello"
```
