# openbnc Config Reference (Operator-Oriented)
...
1. `OPENBNC_WORKSPACE` override (if set)
2. persisted `~/.openbnc/active_workspace.toml` marker (if present)
3. default `~/.openbnc/config.toml`

openbnc logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `openbnc config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name` | `openbnc` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `openbnc doctor traces --limit 20`
  - `openbnc doctor traces --event tool_call_result --contains \"error\"`
  - `openbnc doctor traces --id <trace-id>`
...
1. `OPENBNC_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `OPENBNC_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
...
- When `enabled = true` and no OTP secret exists, openbnc generates one and prints an enrollment URI once.
...
| `state_file` | `~/.openbnc/estop-state.json` | Persistent estop state path |
...
- Use CLI command `openbnc estop` to engage and `openbnc estop resume` to clear levels.
...
- Security-first default: openbnc does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `OPENBNC_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `OPENBNC_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `OPENBNC_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
- Precedence for enable flag: `OPENBNC_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` in `config.toml` → default `false`.
...
- Skill loading and `openbnc skills install` both apply a static security audit. Skills that contain symlinks, script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
- openbnc requests Composio v3 tools with `toolkit_versions=latest` and executes tools with `version="latest"` to avoid stale default tool revisions.
...
3. Validate with `openbnc doctor` before restart/rollout.
...
- While `openbnc channel start` is running, updates to `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url`, and `reliability.*` are hot-applied from `config.toml` on the next inbound message.
...
- `OPENBNC_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
...
- `OPENBNC_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
