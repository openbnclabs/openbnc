...
3. Generic fallback env vars: `OPENBNC_API_KEY` then `API_KEY`
...
- After multimodal normalization, openbnc sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, openbnc returns a structured capability error instead of silently ignoring images.
...
- openbnc normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `openbnc models refresh --provider llamacpp`
...
- Model discovery: `openbnc models refresh --provider sglang`
...
- Model discovery: `openbnc models refresh --provider vllm`
...
- Model discovery: `openbnc models refresh --provider osaurus`
...
- Model discovery: `openbnc models refresh --provider nvidia`
