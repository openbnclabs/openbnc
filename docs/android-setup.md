...
openbnc provides prebuilt binaries for Android devices.
...
The easiest way to run openbnc on Android is via [Termux](https://termux.dev/).
...
# For 64-bit (aarch64):
curl -LO https://github.com/openbnclabs/openbnc/releases/latest/download/openbnc-aarch64-linux-android.tar.gz
tar xzf openbnc-aarch64-linux-android.tar.gz

# For 32-bit (armv7):
curl -LO https://github.com/openbnclabs/openbnc/releases/latest/download/openbnc-armv7-linux-androideabi.tar.gz
tar xzf openbnc-armv7-linux-androideabi.tar.gz
```
...
For advanced users who want to run openbnc outside Termux:

```bash
# From your computer with ADB
adb push openbnc /data/local/tmp/
adb shell chmod +x /data/local/tmp/openbnc
adb shell /data/local/tmp/openbnc --version
```
