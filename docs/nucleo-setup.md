# openbnc on Nucleo-F401RE — Step-by-Step Guide

Run openbnc on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

openbnc can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
openbnc includes everything for Nucleo-F401RE:
...
| Flash command | `openbnc peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via openbnc

From the openbnc repo root:

```bash
openbnc peripheral flash-nucleo
```

This builds `firmware/openbnc-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.
...
| 5 | `openbnc daemon` or `openbnc agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `openbnc peripheral list` to verify.
