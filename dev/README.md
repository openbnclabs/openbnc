# openbnc Development Environment

A fully containerized development sandbox for openbnc agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`openbnc-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `openbnc` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/openbnc-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.openbnc` (mounted to `/openbnc-data/.openbnc`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.openbnc` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `openbnc-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=openbnc-target`) to speed repeat local image builds.
