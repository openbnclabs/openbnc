...
This guide covers testing the Telegram channel integration for openbnc.
...
6. **Error logging**
    ...
    ```bash
    RUST_LOG=debug openbnc channel start
    ```
...
Solution: Check user allowlist
  ...
  4. Run: openbnc onboard --channels-only
```
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time openbnc channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep openbnc`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/openbnc` |
...
```bash
# Maximum logging
RUST_LOG=trace openbnc channel start
...
# Validate config
cat ~/.openbnc/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [openbnc Main README](README.md)
- [Contributing Guide](CONTRIBUTING.md)
- [Issue Tracker](https://github.com/theonlyhennygod/openbnc/issues)
