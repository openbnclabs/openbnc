...
A complete toolkit for building AI-powered robots with openbnc. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build openbnc with robot tools

```bash
# Clone and build
git clone https://github.com/openbnclabs/openbnc
cd openbnc
cargo build -p openbnc-robot-kit --release
```
...
# Copy config
mkdir -p ~/.openbnc
cp crates/robot-kit/robot.toml ~/.openbnc/
cp crates/robot-kit/SOUL.md ~/.openbnc/workspace/
...
# Test in mock mode
./target/release/openbnc agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/openbnc agent -m "Move forward 1 meter"
```
...
```rust
use openbnc_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `openbnc`, add a thin adapter that maps this
...
mkdir openbnc-robot-kit
cp -r target/release/openbnc openbnc-robot-kit/
cp -r examples/robot_kit openbnc-robot-kit/
cp -r ~/.openbnc openbnc-robot-kit/dot-openbnc

# Include models
mkdir -p openbnc-robot-kit/models
cp ~/.openbnc/models/ggml-base.bin openbnc-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf openbnc-robot-kit.tar.gz openbnc-robot-kit/

# Copy to USB
cp openbnc-robot-kit.tar.gz /media/usb/TarBalls/
```
