...
Complete guide to setting up a openbnc-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.openbnc/models
cp models/ggml-base.bin ~/.openbnc/models/
```
...
mkdir -p ~/.openbnc/models/piper
cd ~/.openbnc/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.openbnc/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build openbnc Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/theonlyhennygod/openbnc
cd openbnc

# Build robot kit
cargo build --release -p openbnc-robot-kit

# Build main openbnc (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.openbnc
nano ~/.openbnc/robot.toml
```

```toml
# ~/.openbnc/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.openbnc/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/openbnc_sensors.fifo"
...
```bash
# Configure openbnc to use robot tools
cat > ~/.openbnc/config.toml << 'EOF'
...
cp ~/openbnc/crates/robot-kit/SOUL.md ~/.openbnc/workspace/

# Start agent
./target/release/openbnc agent
```
...
# Start sensor loop
if [ ! -p /tmp/openbnc_sensors.fifo ]; then
    mkfifo /tmp/openbnc_sensors.fifo
fi
...
```bash
# /etc/systemd/system/openbnc-robot.service
sudo tee /etc/systemd/system/openbnc-robot.service << 'EOF'
[Unit]
Description=openbnc Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/openbnc
...
sudo systemctl enable openbnc-robot
sudo systemctl start openbnc-robot

# Check status
sudo systemctl status openbnc-robot
journalctl -u openbnc-robot -f  # View logs
```
