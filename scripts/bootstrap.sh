...
usage() {
  cat <<'USAGE'
openbnc installer bootstrap engine

Usage:
  ./openbnc_install.sh [options]
  ./bootstrap.sh [options]         # compatibility entrypoint

Modes:
  Default mode installs/builds openbnc only (requires existing Rust toolchain).
...
Examples:
  ./openbnc_install.sh
  ./openbnc_install.sh --guided
  ./openbnc_install.sh --install-system-deps --install-rust
  ./openbnc_install.sh --prefer-prebuilt
  ./openbnc_install.sh --prebuilt-only
  ./openbnc_install.sh --onboard --api-key "sk-..." --provider openrouter [--model "openrouter/auto"]
  ./openbnc_install.sh --interactive-onboard

  # Compatibility entrypoint:
  ./bootstrap.sh --docker

  # Remote one-liner
  curl -fsSL https://raw.githubusercontent.com/openbnclabs/openbnc/main/scripts/bootstrap.sh | bash

Environment:
  OPENBNC_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  OPENBNC_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  OPENBNC_DOCKER_IMAGE      Docker image tag to build/run (default: openbnc-bootstrap:local)
  OPENBNC_API_KEY           Used when --api-key is not provided
  OPENBNC_PROVIDER          Used when --provider is not provided (default: openrouter)
  OPENBNC_MODEL             Used when --model is not provided
  OPENBNC_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  OPENBNC_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  OPENBNC_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${OPENBNC_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${OPENBNC_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
install_prebuilt_binary() {
  ...
  archive_url="https://github.com/openbnclabs/openbnc/releases/latest/download/openbnc-${target}.tar.gz"
  temp_dir="$(mktemp -d -t openbnc-prebuilt-XXXXXX)"
  archive_path="$temp_dir/openbnc-${target}.tar.gz"
  ...
  extracted_bin="$temp_dir/openbnc"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name openbnc -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable openbnc binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/openbnc"
  rm -rf "$temp_dir"

  info "Installed pre-built binary to $install_dir/openbnc"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/openbnc-pacman.XXXXXX.conf)"
...
run_guided_installer() {
  ...
  if prompt_yes_no "Install openbnc into cargo bin now?" "yes"; then
    SKIP_INSTALL=false
  else
    SKIP_INSTALL=true
  fi
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${OPENBNC_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set OPENBNC_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set OPENBNC_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${OPENBNC_DOCKER_IMAGE:-openbnc-bootstrap:local}"
  fallback_image="ghcr.io/openbnclabs/openbnc:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.openbnc-docker"
  else
    default_data_dir="$WORK_DIR/.openbnc-docker"
  fi
  docker_data_dir="${OPENBNC_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.openbnc" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official openbnc image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.openbnc:/openbnc-data/.openbnc"
  workspace_mount="$docker_data_dir/workspace:/openbnc-data/workspace"
...
Use either:
  --api-key "sk-..."
or:
  OPENBNC_API_KEY="sk-..." ./openbnc_install.sh --docker
or run interactive:
  ./openbnc_install.sh --docker --interactive-onboard
MSG
      exit 1
    fi
    if [[ -n "$MODEL" ]]; then
  ...
  "$CONTAINER_CLI" run --rm -it \
    ...
    -e HOME=/openbnc-data \
    -e OPENBNC_WORKSPACE=/openbnc-data/workspace \
    -v "$config_mount" \
    -v "$workspace_mount" \
    "$docker_image" \
    "${onboard_cmd[@]}"
...
REPO_URL="https://github.com/openbnclabs/openbnc.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
CONTAINER_CLI="${OPENBNC_CONTAINER_CLI:-docker}"
API_KEY="${OPENBNC_API_KEY:-}"
PROVIDER="${OPENBNC_PROVIDER:-openrouter}"
MODEL="${OPENBNC_MODEL:-}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${OPENBNC_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set OPENBNC_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t openbnc-bootstrap-XXXXXX)"
...
Your containerized openbnc data is persisted under:
DONE
...
Next steps:
  ./openbnc_install.sh --docker --interactive-onboard
  ./openbnc_install.sh --docker --api-key "sk-..." --provider openrouter
DONE
  exit 0
fi
...
or re-run with:
  ./openbnc_install.sh --install-rust
MSG
  exit 1
fi
...
if [[ "$SKIP_INSTALL" == false ]]; then
  info "Installing openbnc to cargo bin"
...
OPENBNC_BIN=""
if have_cmd openbnc; then
  OPENBNC_BIN="openbnc"
elif [[ -x "$HOME/.cargo/bin/openbnc" ]]; then
  OPENBNC_BIN="$HOME/.cargo/bin/openbnc"
elif [[ -x "$WORK_DIR/target/release/openbnc" ]]; then
  OPENBNC_BIN="$WORK_DIR/target/release/openbnc"
fi

if [[ "$RUN_ONBOARD" == true ]]; then
  if [[ -z "$OPENBNC_BIN" ]]; then
    error "onboarding requested but openbnc binary is not available."
    error "Run without --skip-install, or ensure openbnc is in PATH."
    exit 1
  fi

  if [[ "$INTERACTIVE_ONBOARD" == true ]]; then
    info "Running interactive onboarding"
    "$OPENBNC_BIN" onboard --interactive
  else
    if [[ -z "$API_KEY" ]]; then
      cat <<'MSG'
==> Onboarding requested, but API key not provided.
Use either:
  --api-key "sk-..."
or:
  OPENBNC_API_KEY="sk-..." ./openbnc_install.sh --onboard
or run interactive:
  ./openbnc_install.sh --interactive-onboard
MSG
      exit 1
    ...
    ONBOARD_CMD=("$OPENBNC_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    "${ONBOARD_CMD[@]}"
