//! Performance benchmarks for openbnc hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: https://github.com/openbnclabs/openbnc/issues/618 (item 7)

use criterion::{black_box, criterion_group, criterion_main, Criterion};
use std::sync::{Arc, Mutex};

use openbnc::agent::agent::Agent;
use openbnc::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use openbnc::config::MemoryConfig;
use openbnc::memory;
use openbnc::memory::{Memory, MemoryCategory};
use openbnc::observability::{NoopObserver, Observer};
use openbnc::providers::{ChatRequest, ChatResponse, Provider, ToolCall};
use openbnc::tools::{Tool, ToolResult};
...
<tool_call>
{"name": "search", "arguments": {"query": "openbnc architecture"}}
</tool_call>
    ...
    let response = ChatResponse {
        ...
        tool_calls: vec![
            ToolCall {
                id: "tc1".into(),
                name: "search".into(),
                arguments: r#"{"query": "openbnc"}"#.into(),
            },
            ToolCall {
                id: "tc2".into(),
                name: "read_file".into(),
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about openbnc agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("openbnc agent"), 10, None)
                    .await
                    .unwrap()
            })
        });
