...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/theonlyhennygod/openbnc/security/advisories/new)
...
openbnc implements defense-in-depth security:
...
openbnc Docker images follow CIS Docker Benchmark best practices:
...
docker build -t openbnc .
docker inspect --format='{{.Config.User}}' openbnc
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace openbnc gateway
```
