# CLAUDE.md — openbnc Agent Engineering Protocol
...
openbnc is a Rust-first autonomous agent runtime optimized for:
...
- If identity-like naming is required in tests/examples, use openbnc-native labels only (`openbncAgent`, `openbnc_user`, `openbnc_node`).
...
- If identity-like context is unavoidable, use openbnc-scoped roles/labels only (for example: `openbncAgent`, `openbncOperator`, `openbnc_user`) and avoid real-world personas.
- Recommended identity-safe naming palette (use when identity-like context is required):
    - actor labels: `openbncAgent`, `openbncOperator`, `openbncMaintainer`, `openbnc_user`
    - service/runtime labels: `openbnc_bot`, `openbnc_service`, `openbnc_runtime`, `openbnc_node`
    - environment labels: `openbnc_project`, `openbnc_workspace`, `openbnc_channel`
