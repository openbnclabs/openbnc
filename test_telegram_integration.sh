#!/bin/bash
# openbnc Telegram Integration Test Suite
...
if [ -f "target/release/openbnc" ]; then
    BINARY_SIZE=$(ls -lh target/release/openbnc | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/openbnc 2>/dev/null || stat -c%s target/release/openbnc)
...
CONFIG_PATH="$HOME/.openbnc/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'openbnc onboard' first"
    fi
else
    warn "No config file found - run 'openbnc onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/openbnc channel doctor 2>&1 || true)
END_TIME=$(date +%s)
