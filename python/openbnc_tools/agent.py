...
SYSTEM_PROMPT = """You are openbnc, an AI assistant with tool access. Use tools to accomplish tasks.
...
class OpenbncAgent:
    ...
    def invoke(self, input: dict[str, Any], config: Optional[dict] = None) -> dict:
        ...
        raise RuntimeError(
            "OpenbncAgent.invoke() cannot be called inside an active event loop. "
            "Use 'await OpenbncAgent.ainvoke(...)' instead."
        )
...
def create_agent(
    ...
    """
    Create a openbnc agent with LangGraph-based tool calling.
    ...
    Returns:
        Configured OpenbncAgent instance

    Example:
        ```python
        from openbnc_tools import create_agent, shell, file_read
