...
Python companion package for [openbnc](https://github.com/openbnclabs/openbnc) — LangGraph-based tool calling for consistent LLM agent execution.
...
```bash
pip install openbnc-tools[discord]
```
...
# Run the CLI
openbnc-tools "List files in the current directory"
...
```python
from openbnc_tools import tool
...
| Feature | Rust openbnc | openbnc-tools |
|---------|---------------|----------------|
...
Use **Rust openbnc** for production edge deployments. Use **openbnc-tools** when you need guaranteed tool calling consistency or Python ecosystem integration.
