...
Examples:
  ...
  openbnc channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  openbnc channel bind-telegram 123456789")]
    BindTelegram {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `openbnc` workspace
    Openclaw {
...
Examples:
  ...
  openbnc cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  openbnc cron add-every 60000 'Ping heartbeat'     # every minute
  openbnc cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  openbnc cron once 2h 'Follow up on deployment'
  openbnc cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  openbnc cron update <task-id> --expression '0 8 * * *'
  openbnc cron update <task-id> --tz Europe/London --name 'Morning check'
  openbnc cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  openbnc hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  openbnc peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash openbnc firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash openbnc firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  openbnc peripheral flash
  openbnc peripheral flash --port /dev/cu.usbmodem12345
  openbnc peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash openbnc firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
