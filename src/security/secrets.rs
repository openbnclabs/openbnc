...
// in `~/.openbnc/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.openbnc/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(openbnc_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: openbnc_dir.join(".secret_key"),
            enabled,
        }
    }
