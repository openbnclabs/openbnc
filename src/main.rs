...
/// `openbnc` - Bridging crypto infrastructure and autonomous AI agents.
#[derive(Parser, Debug)]
#[command(name = "openbnc")]
#[command(author = "theonlyhennygod")]
#[command(version)]
#[command(about = "Bridging the gap between crypto infrastructure and autonomous AI agents.", long_about = None)]
struct Cli {
...
Examples:
  openbnc agent                              # interactive session
  openbnc agent -m \"Summarize today's logs\"  # single message
  openbnc agent -p anthropic --model claude-sonnet-4-20250514
  openbnc agent --peripheral nucleo-f401re:/dev/ttyACM0")]
    Agent {
...
Examples:
  openbnc gateway                  # use config defaults
  openbnc gateway -p 8080          # listen on port 8080
  openbnc gateway --host 0.0.0.0   # bind to all interfaces
  openbnc gateway -p 0             # random available port")]
    Gateway {
...
Launches the full openbnc runtime: gateway server, all configured \
channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and \
the cron scheduler. This is the recommended way to run openbnc in \
production or as an always-on assistant.

Use 'openbnc service install' to register the daemon as an OS \
service (systemd/launchd) for auto-start on boot.

Examples:
  openbnc daemon                   # use config defaults
  openbnc daemon -p 9090           # gateway on port 9090
  openbnc daemon --host 127.0.0.1  # localhost only")]
    Daemon {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
    ...
    /// - `openbnc estop --level domain-block --domain "*.chase.com"`
    ...
    Estop {
        ...
        /// Level used when engaging estop from `openbnc estop`.
        #[arg(long, value_enum)]
...
Examples:
  openbnc cron list
  openbnc cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York
  openbnc cron add '*/30 * * * *' 'Check system health'
  openbnc cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  openbnc cron add-every 60000 'Ping heartbeat'
  openbnc cron once 30m 'Run backup in 30 minutes'
  openbnc cron pause <task-id>
  openbnc cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    Cron {
        #[command(subcommand)]
...
Add, remove, list, and health-check channels that connect openbnc \
...
Examples:
  ...
  openbnc channel add telegram '{\"bot_token\":\"...\",\"name\":\"my-bot\"}'
  openbnc channel remove my-bot
  openbnc channel bind-telegram openbnc_user")]
    Channel {
        #[command(subcommand)]
...
Examples:
  ...
  openbnc hardware introspect /dev/ttyACM0
  openbnc hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
        hardware_command: openbnc::HardwareCommands,
    },
...
Examples:
  ...
  openbnc peripheral add nucleo-f401re /dev/ttyACM0
  openbnc peripheral add rpi-gpio native
  openbnc peripheral flash --port /dev/cu.usbmodem12345
  openbnc peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
        peripheral_command: openbnc::PeripheralCommands,
    },
...
Examples:
  openbnc memory stats
  openbnc memory list
  openbnc memory list --category core --limit 10
  openbnc memory get <key>
  openbnc memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
...
Examples:
  openbnc config schema              # print JSON Schema to stdout
  openbnc config schema > schema.json")]
    Config {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Generate shell completion scripts for `openbnc`.

The script is printed to stdout so it can be sourced directly:

Examples:
  source <(openbnc completions bash)
  openbnc completions zsh > ~/.zfunc/_openbnc
  openbnc completions fish > ~/.config/fish/completions/openbnc.fish")]
    Completions {
        /// Target shell
        #[arg(value_enum)]
...
async fn main() -> Result<()> {
    ...
    if let Some(config_dir) = &cli.config_dir {
        ...
        std::env::set_var("OPENBNC_CONFIG_DIR", config_dir);
    }
    ...
    } = &cli.command
    {
        ...
        if std::env::var("OPENBNC_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!("Initialized OTP secret for openbnc.");
    ...
    match cli.command {
        ...
        Commands::Gateway { port, host } => {
            ...
            if port == 0 {
                info!("🚀 Starting openbnc Gateway on {host} (random port)");
            } else {
                info!("🚀 Starting openbnc Gateway on {host}:{port}");
            }
        ...
        Commands::Daemon { port, host } => {
            ...
            let host = host.unwrap_or_else(|| config.gateway.host.clone());
            if port == 0 {
                info!("🧠 Starting openbnc Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting openbnc Daemon on {host}:{port}");
            }
            daemon::run(config, host, port).await
        }

        Commands::Status => {
            println!("openbnc Status");
            println!();
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `openbnc auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `openbnc auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
            ...
            match provider.as_str() {
                "openai-codex" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `openbnc auth login --provider openai-codex` first."
                        )
                    })?;
                ...
                "gemini" => {
                    let pending = load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending Gemini login found. Run `openbnc auth login --provider gemini` first."
                        )
                    })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `openbnc auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `openbnc auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["openbnc", "completions", shell])
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["openbnc", "onboard", "--force"])
    ...
    fn cli_parses_estop_default_engage() {
        let cli = Cli::try_parse_from(["openbnc", "estop"]).expect("estop command should parse");
    ...
    fn cli_parses_estop_resume_domain() {
        let cli = Cli::try_parse_from(["openbnc", "estop", "resume", "--domain", "*.chase.com"])
