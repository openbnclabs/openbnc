...
/// Handle `openbnc peripheral` subcommands.
    ...
    match cmd {
        crate::PeripheralCommands::List => {
            ...
            if boards.is_empty() {
                ...
                println!();
                println!("Add one with: openbnc peripheral add <board> <path>");
                println!("  Example: openbnc peripheral add nucleo-f401re /dev/ttyACM0");
                println!();
