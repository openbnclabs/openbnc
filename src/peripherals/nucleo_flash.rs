//! Flash openbnc Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash openbnc Nucleo firmware. Builds from firmware/openbnc-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (openbnc's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("openbnc-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from openbnc repo root.",
            firmware_dir.display()
        );
    }

    println!("Building openbnc Nucleo firmware...");
    ...
    println!("openbnc Nucleo firmware flashed successfully.");
