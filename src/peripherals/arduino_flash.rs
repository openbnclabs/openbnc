//! Flash openbnc Arduino firmware via arduino-cli.
//!
...
/// openbnc Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/openbnc-arduino/openbnc-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "openbnc-arduino";
...
/// Flash openbnc firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("openbnc_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling openbnc Arduino firmware...");
    ...
    println!("openbnc firmware flashed successfully.");
