...
const SERVICE_LABEL: &str = "com.openbnc.daemon";
const WINDOWS_TASK_NAME: &str = "openbnc daemon";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "openbnc.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["openbnc", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "openbnc.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["openbnc", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "openbnc.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["openbnc", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["openbnc", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/openbnc");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/openbnc");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "openbnc", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove openbnc from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/openbnc)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
    ...
    println!("   Start with: openbnc service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    let exe = std::env::current_exe().context("Failed to resolve current executable")?;
    let unit = format!(
        "[Unit]\nDescription=openbnc daemon\nAfter=network.target\n\n[Service]\nType=simple\nExecStart={} daemon\nRestart=always\nRestartSec=3\n\n[Install]\nWantedBy=default.target\n",
        exe.display()
    );

    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "openbnc.service"]));
...
/// Check if the openbnc user exists and has expected properties.
...
fn check_openbnc_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "openbnc"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser openbnc && delgroup openbnc",
            "addgroup -S openbnc && adduser -S -s /sbin/nologin -H -D -G openbnc openbnc",
        )
    } else {
        ("userdel openbnc", "useradd -r -s /sbin/nologin openbnc")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'openbnc' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'openbnc' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/openbnc" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: openbnc user has home directory '{}' (expected /var/lib/openbnc or /nonexistent)",
                        home
                    );
                }
...
fn ensure_openbnc_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "openbnc"]).output();
    if let Ok(output) = output {
        if output.status.success() {
            return check_openbnc_user();
        }
    }
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "openbnc"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "openbnc"])
                .output()
                .context("Failed to create openbnc group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create openbnc group: {}", stderr.trim());
            }
            println!("✅ Created system group: openbnc");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create openbnc user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create openbnc user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "openbnc"])
            .output()
            .context("Failed to create openbnc user")?;
    ...
    println!("✅ Created system user: openbnc");
    Ok(())
}

/// Change ownership of a path to openbnc:openbnc
#[cfg(unix)]
fn chown_to_openbnc(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["openbnc:openbnc", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to openbnc:openbnc: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_openbnc(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_openbnc(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "openbnc:openbnc", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to openbnc:openbnc: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_openbnc(_path: &Path) -> Result<()> {
    Ok(())
}
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(PathBuf::from(fields[5]).join(".openbnc"));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'openbnc' cannot write {} ({details}). \
             Re-run `sudo openbnc service install` and ensure ownership is openbnc:openbnc.",
            path.display(),
        );
    }
...
command_user="openbnc:openbnc"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/openbnc/access.log"
error_log="/var/log/openbnc/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/openbnc");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo openbnc service install"
        );
    }
    ...
    let config_dir = Path::new("/etc/openbnc");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/openbnc");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_openbnc(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by openbnc:openbnc)",
    ...
    chown_recursive_to_openbnc(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/openbnc");
    ...
    run_checked(Command::new("rc-update").args(["add", "openbnc", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/openbnc");
    println!("   Config path: /etc/openbnc/config.toml");
    println!("   Start with: sudo openbnc service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("openbnc-daemon.cmd");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/openbnc.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "openbnc daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/openbnc");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/openbnc"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"openbnc\""));
        assert!(script.contains("description=\"openbnc daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/openbnc\""));
        assert!(script.contains("command_args=\"--config-dir /etc/openbnc daemon\""));
        assert!(!script.contains("env OPENBNC_CONFIG_DIR"));
        assert!(!script.contains("env OPENBNC_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"openbnc:openbnc\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/openbnc/access.log\""));
        assert!(script.contains("error_log=\"/var/log/openbnc/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/openbnc");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/openbnc");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/openbnc");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/openbnc"), true);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/openbnc/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/openbnc/workspace'".to_string(),
                "openbnc".to_string()
            ]
        );
    }
