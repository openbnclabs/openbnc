...
//! openbnc to external platforms. Each channel implements the [`Channel`] trait
...
const SYSTEMD_STATUS_ARGS: [&str; 3] = ["--user", "is-active", "openbnc.service"];
const SYSTEMD_RESTART_ARGS: [&str; 3] = ["--user", "restart", "openbnc.service"];
const OPENRC_STATUS_ARGS: [&str; 2] = ["openbnc", "status"];
const OPENRC_RESTART_ARGS: [&str; 2] = ["openbnc", "restart"];
...
async fn load_runtime_defaults_from_config_file(path: &Path) -> Result<ChannelRuntimeDefaults> {
    ...
    if let Some(openbnc_dir) = path.parent() {
        let store = crate::security::SecretStore::new(openbnc_dir, parsed.secrets.encrypt);
...
fn build_models_help_response(current: &ChannelRouteSelection, workspace_dir: &Path) -> String {
    ...
    if cached_models.is_empty() {
        let _ = writeln!(
            response,
            "\nNo cached model list found for `{}`. Ask the operator to run `openbnc models refresh --provider {}`.",
...
pub fn build_system_prompt_with_mode(
    ...
    if prompt.is_empty() {
        "You are openbnc, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct."
            .to_string()
    } else {
        prompt
    }
...
async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels_config.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `openbnc onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `openbnc daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `openbnc service stop && openbnc service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        if !listed.contains("com.openbnc.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.openbnc.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.openbnc.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let openrc_init_script = PathBuf::from("/etc/init.d/openbnc");
...
pub(crate) async fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: openbnc channel start");
            println!("To check health:    openbnc channel doctor");
            println!("To configure:      openbnc onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `openbnc onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.openbnc/config.toml directly");
        }
...
fn collect_configured_channels(
    ...
    if let Some(ref mx) = config.channels_config.matrix {
        channels.push(ConfiguredChannel {
            display_name: "Matrix",
            channel: Arc::new(MatrixChannel::new_with_session_hint_and_openbnc_dir(
                mx.homeserver.clone(),
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `openbnc onboard` first.");
        return Ok(());
    }

    println!("🩺 openbnc Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `openbnc gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let provider_runtime_options = providers::ProviderRuntimeOptions {
        ...
        openbnc_dir: config.config_path.parent().map(std::path::PathBuf::from),
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `openbnc onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 openbnc Channel Server");
    println!("  🤖 Model:    {model}");
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn process_channel_message_uses_runtime_default_model_from_store() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            reliability: Arc::new(crate::config::ReliabilityConfig::default()),
            provider_runtime_options: providers::ProviderRuntimeOptions {
                openbnc_dir: Some(temp.path().to_path_buf()),
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from openbnc 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
...
This is an example JSON object for profile settings."#;
    ...
    fn maybe_restart_daemon_systemd_args_regression() {
        assert_eq!(
            SYSTEMD_STATUS_ARGS,
            ["--user", "is-active", "openbnc.service"]
        );
        assert_eq!(
            SYSTEMD_RESTART_ARGS,
            ["--user", "restart", "openbnc.service"]
        );
    }

    #[test]
    fn maybe_restart_daemon_openrc_args_regression() {
        assert_eq!(OPENRC_STATUS_ARGS, ["openbnc", "status"]);
        assert_eq!(OPENRC_RESTART_ARGS, ["openbnc", "restart"]);
    }
