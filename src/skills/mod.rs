...
const OPEN_SKILLS_SYNC_MARKER: &str = ".openbnc-open-skills-sync";
const OPEN_SKILLS_SYNC_INTERVAL_SECS: u64 = 60 * 60 * 24 * 7;

/// A skill is a user-defined or community-built capability.
/// Skills live in `~/.openbnc/workspace/skills/<name>/SKILL.md`
...
fn open_skills_enabled(config_open_skills_enabled: Option<bool>) -> bool {
    let env_override = std::env::var("OPENBNC_OPEN_SKILLS_ENABLED").ok();
...
fn resolve_open_skills_dir(config_open_skills_dir: Option<&str>) -> Option<PathBuf> {
    let env_dir = std::env::var("OPENBNC_OPEN_SKILLS_DIR").ok();
    ...
    match command {
        crate::SkillCommands::List => {
            ...
            if skills.is_empty() {
                ...
                println!();
                println!("  Create one: mkdir -p ~/.openbnc/workspace/skills/my-skill");
                println!("              echo '# My Skill' > ~/.openbnc/workspace/skills/my-skill/SKILL.md");
                println!();
                println!("  Or install: openbnc skills install <source>");
            } else {
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.openbnc");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.openbnc/skills"));
    }
    ...
    fn load_skills_with_config_reads_open_skills_dir_without_network() {
        ...
        let _enabled_guard = EnvVarGuard::unset("OPENBNC_OPEN_SKILLS_ENABLED");
        let _dir_guard = EnvVarGuard::unset("OPENBNC_OPEN_SKILLS_DIR");
