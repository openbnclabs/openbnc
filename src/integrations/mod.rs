...
fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `openbnc onboard --interactive` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: openbnc onboard --channels-only");
            println!("    4. Start: openbnc channel start");
        }
        "Discord" => {
            println!("  Setup:");
            ...
            println!("    4. Run: openbnc onboard --channels-only");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "Browser" => {
            println!("  Built-in:");
            println!("    openbnc can control Chrome/Chromium for web tasks.");
            println!("    Uses headless browser automation.");
        }
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.openbnc/workspace/cron/");
            println!("    Run: openbnc cron list");
        }
        "Webhooks" => {
            println!("  Built-in:");
            println!("    HTTP endpoint for external triggers.");
            println!("    Run: openbnc gateway");
        }
        _ => {
            if status == IntegrationStatus::ComingSoon {
                println!("  This integration is planned. Stay tuned!");
                println!("  Track progress: https://github.com/theonlyhennygod/openbnc");
            }
        }
    }
