...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to openbnc — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 9, "Channels (How You Talk to openbnc)");
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("OPENBNC_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub async fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to openbnc)");
...
/// Use `openbnc onboard` or `openbnc onboard --api-key sk-... --provider openrouter --memory sqlite|lucid`.
/// Use `openbnc onboard --interactive` for the full wizard.
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    ...
    let config_dir = home.join(".openbnc");
...
async fn run_quick_setup_with_home(
    ...
    let (openbnc_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    ...
    if credential_override.is_none() {
        ...
        println!("    2. Or edit:           ~/.openbnc/config.toml");
        println!("    3. Chat:              openbnc agent -m \"Hello!\"");
        println!("    4. Gateway:           openbnc gateway");
    } else {
        println!("    1. Chat:     openbnc agent -m \"Hello!\"");
        println!("    2. Gateway:  openbnc gateway");
        println!("    3. Status:   openbnc status");
    }
    println!();
...
pub async fn run_models_refresh(
    ...
    if !force {
        ...
        .await?
        {
            ...
            println!();
            println!(
                "Tip: run `openbnc models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet("openbnc works with ANY API that speaks the OpenAI chat completions format.");
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("openbnc will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how openbnc connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("openbnc uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("openbnc can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("openbnc can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::new()
        .with_prompt("  How should openbnc interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how openbnc stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to openbnc from anywhere.");
        ...
        match choice {
            ChannelMenuChoice::Telegram => {
                ...
                println!(
                    ...
                    style("— talk to openbnc from Telegram").dim()
                );
            ...
            ChannelMenuChoice::Discord => {
                ...
                println!(
                    ...
                    style("— talk to openbnc from Discord").dim()
                );
            ...
            ChannelMenuChoice::Slack => {
                ...
                println!(
                    ...
                    style("— talk to openbnc from Slack").dim()
                );
            ...
            ChannelMenuChoice::IMessage => {
                ...
                print_bullet("openbnc reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::new()
                        ...
                        .default("~/.openbnc/state/whatsapp-web/session.db".into())
                        .interact_text()?;
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("openbnc-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via OPENBNC_WHATSAPP_APP_SECRET env var
                    session_path: None,
                    pair_phone: None,
                    pair_code: None,
                    allowed_numbers,
            ...
            ChannelMenuChoice::Lark | ChannelMenuChoice::Feishu => {
                ...
                println!(
                    ...
                    style(format!("— talk to openbnc from {provider_label}")).dim()
                );
                print_bullet(&format!(
            ...
            ChannelMenuChoice::Nostr => {
                ...
                print_bullet("openbnc will listen for encrypted DMs on Nostr relays.");
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("openbnc is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if config.api_key.is_none() && !provider_supports_keyless_local_usage(provider) {
        if provider == "openai-codex" {
            ...
            println!(
                "       {}",
                style("openbnc auth login --provider openai-codex --device-code").yellow()
            );
            ...
            println!(
                "       {}",
                style(
                    "or: openbnc auth paste-token --provider anthropic --auth-kind authorization"
                )
                .yellow()
            );
        } else {
    ...
    if has_channels {
        ...
        println!("       {}", style("openbnc channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("openbnc agent -m \"Hello, openbnc!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("openbnc agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("openbnc status").yellow());
...
mod tests {
    ...
    async fn quick_setup_model_override_persists_to_config_toml() {
        ...
        let _workspace_env = EnvVarGuard::unset("OPENBNC_WORKSPACE");
        let _config_env = EnvVarGuard::unset("OPENBNC_CONFIG_DIR");
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        ...
        let _config_env = EnvVarGuard::unset("OPENBNC_CONFIG_DIR");
        let tmp = TempDir::new().unwrap();
        let openbnc_dir = tmp.path().join(".openbnc");
        let config_path = openbnc_dir.join("config.toml");

        tokio::fs::create_dir_all(&openbnc_dir).await.unwrap();
    ...
    async fn quick_setup_respects_zero_claw_workspace_env_layout() {
        let _env_guard = env_lock().lock().await;
        let tmp = TempDir::new().unwrap();
        let workspace_root = tmp.path().join("openbnc-data");
        let workspace_dir = workspace_root.join("workspace");
        let expected_config_path = workspace_root.join(".openbnc").join("config.toml");
        ...
        .await
        .expect("quick setup should honor OPENBNC_WORKSPACE");
    ...
    async fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            ...
            .unwrap();
        assert!(
            identity.contains("**Name:** openbnc"),
            "should default agent name to openbnc"
        );
    ...
    async fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = tokio::fs::read_to_string(tmp.path().join("SOUL.md"))
            .await
            .unwrap();
        assert!(soul.contains("openbnc-v2"));
    }
