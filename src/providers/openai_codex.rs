...
const DEFAULT_CODEX_INSTRUCTIONS: &str =
    "You are openbnc, a concise and helpful coding assistant.";
...
impl OpenAiCodexProvider {
    pub fn new(options: &ProviderRuntimeOptions) -> Self {
        let state_dir = options
            .openbnc_dir
            .clone()
            .unwrap_or_else(default_openbnc_dir);
...
fn default_openbnc_dir() -> PathBuf {
...
fn resolve_reasoning_effort(model_id: &str) -> String {
    let raw = std::env::var("OPENBNC_CODEX_REASONING_EFFORT")
        .ok()
...
impl OpenAiCodexProvider {
    async fn send_responses_request(
        ...
        let access_token = self
            ...
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex auth profile not found. Run `openbnc auth login --provider openai-codex`."
                )
            })?;
        let account_id = profile
            ...
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex account id not found in auth profile/token. Run `openbnc auth login --provider openai-codex` again."
                )
            })?;
